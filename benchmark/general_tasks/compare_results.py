#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Compare general tasks (lm-eval) results across models and plot bar charts.

Usage:
    python compare_results.py <output_dir> [output_path]

Directory layout: outputs/general_tasks/{model}/{method}/all_metrics.json
"""
...
# Top-level tasks only (excludes mmlu subtasks)
MAIN_TASKS = [
    'arc_challenge', 'arc_easy', 'boolq', 'cb', 'cola', 'copa',
    'gsm8k', 'hellaswag', 'mathqa', 'mmlu', 'mnli', 'mrpc', 'multirc',
    'openbookqa', 'piqa', 'qnli', 'qqp', 'record', 'rte',
    'sst2', 'wic', 'winogrande', 'wnli', 'wsc'
]
...
def load_result(result_path):
    """Load one all_metrics.json and extract main-task metrics as percentages."""
    with open(result_path, 'r', encoding='utf-8') as f:
        data = json.load(f)

    if 'general_tasks' in data:
        data = data['general_tasks']

    results = {}
    for task in MAIN_TASKS:
        if task in data:
            task_data = data[task]
            metric = TASK_METRICS.get(task, 'acc,none')
...
def find_all_results(output_dir):
    """Discover all_metrics.json files. Supports per-model dir or aggregate dir.
    ...
    # Detect mode: if any subdir has all_metrics.json, treat current as a model dir
    has_direct_results = False
    for subdir in output_path.iterdir():
        if subdir.is_dir() and (subdir / 'all_metrics.json').exists():
            has_direct_results = True
            break

    if has_direct_results:
        # Mode 1: current is a model dir; subdirs are methods
        for method_dir in output_path.iterdir():
            if method_dir.is_dir():
                result_file = method_dir / 'all_metrics.json'
                if result_file.exists():
                    results[method_dir.name] = load_result(result_file)
                    print(f"已加载: {method_dir.name}")
                for param_dir in method_dir.iterdir():
                    if param_dir.is_dir():
                        param_result = param_dir / 'all_metrics.json'
...
def plot_table(results, output_path='table.png', model_name=None):
    ...
    # Union of tasks, sorted with 'avg' last
    all_tasks = set()
    ...
    # Best score per column for bolding
    best_scores = {}
    ...
    title = f'General Tasks Results - {model_name}' if model_name else 'General Tasks Results'
...
def plot_comparison(results, output_path='comparison.png', figsize=(16, 8), model_name=None):
    ...
    max_score = max(max(r.values()) for r in results.values() if r)
    if max_score > 95:
        ax.set_ylim(0, 115)
    else:
        ax.set_ylim(0, 105)
    ...
    title = f'General Tasks Results Comparison - {model_name}' if model_name else 'General Tasks Results Comparison'
...
def main():
    if len(sys.argv) < 2:
        ...
        print("示例: python compare_results.py outputs/general_tasks/llama-3-8b")
        sys.exit(1)
    ...
    if not results:
        print("未找到任何 all_metrics.json 文件！")
        return
