...
def parse_args():
    """Parse args for score computation."""
    p = argparse.ArgumentParser()
    p.add_argument("--task", type=str, required=True, help="Task name or 'all'")
    p.add_argument("--output_dir", type=str, default="outputs/infinitebench")
    p.add_argument("--model_name", type=str, default="Llama-3.1-8B-Instruct")
    p.add_argument("--method", type=str, default="fullkv",
                   choices=["fullkv", "origin", "pyramidkv", "snapkv", "cakekv", "compresskv", "streamingllm", "spotlight", "sparse", "sparsetopk", "sparsetopk_offline", "quest","magicpig","bpc","bpc-offline"])
    p.add_argument("--max_capacity_prompt", type=int, default=512)
    return p.parse_args()
...
if __name__ == "__main__":
    ...
    # Result dir layout matches llama.py:
    # outputs/infinitebench/{model_name}/{method}[/{max_capacity_prompt}]/
    if args.method in ["fullkv", "origin", "sparse", "spotlight", "sparsetopk", "sparsetopk_offline","magicpig","bpc","bpc-offline"]:
        result_dir = Path(args.output_dir, args.model_name, args.method)
    else:
        result_dir = Path(args.output_dir, args.model_name, args.method, str(args.max_capacity_prompt))
    
    print(f"Looking for predictions in: {result_dir}")
    ...
    for task in tasks:
        ...
        if not preds_path.exists():
            # Range-suffixed form, e.g. preds_passkey_0-20.jsonl
            import glob
            pattern = str(result_dir / f"preds_{task}_*.jsonl")
            matches = glob.glob(pattern)
            if matches:
                preds_path = Path(matches[0])
            else:
                print(f"Predictions not found for task {task}: {preds_path}")
                continue
        print(f"\n=== Task: {task} ===")
        preds = list(iter_jsonl(preds_path))
        labels = get_labels(preds)
        pred_strings = get_preds(preds, task)
        
        acc = get_score(labels, pred_strings, task, args.model_name)
        all_scores[task] = acc
