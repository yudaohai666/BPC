...
void fused_quant_pack_probe_hash64_fn(
  at::Tensor probe,
  at::Tensor hashcode,
  at::Tensor scores,
  int T,
  int hash_T
    ...
    fused_quant_pack_probe_hash64(probe, hashcode, scores, T, hash_T);
}

void fused_gather_kv_fn(
  at::Tensor keys,
  at::Tensor values,
  at::Tensor indices,
  at::Tensor selected_keys,
  at::Tensor selected_values
) {
    fused_gather_kv(keys, values, indices, selected_keys, selected_values);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("fused_quant_pack_probe_hash64_fn", &fused_quant_pack_probe_hash64_fn, "fused_quant_pack_probe_hash64_fn with boundary handling");
  m.def("fused_gather_kv_fn", &fused_gather_kv_fn, "fused_gather_kv_fn");
}
